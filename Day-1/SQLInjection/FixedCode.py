...
def init_db():
    ...
    # Check if customers table is empty before inserting fake data
...
def login(username, password):
    ...
    # Secure query using placeholders (prevents SQL injection)
...
# Fetch all customers securely
def get_customers():
    conn = sqlite3.connect('example.db')
    c = conn.cursor()
    c.execute("SELECT id, name, email, phone FROM customers")  # Only fetch needed columns
